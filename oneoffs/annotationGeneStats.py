...
DESCRIPTION -
Version 0.2-20191009 - moved classes to gfftools to import here.
...
parser.add_argument('-A', '--AED_per_gene',
                   action='store_true', default=False,
                   help='''
                        The output here works only for maker GFFs.
                        It completely replaces default output and file writing.
...
### THIS IS NOW BEING DEVELOPED IN gfftools.py AND IS CALLED AT TOP
##
...
def default_output(args, gff):
    ...
    if args.maker or args.full:
        ...
        print "Num genes with 5pUTR AND 3pUTR", gff.get_num_genes_with_5pUTR_AND_3pUTR()
        print "Num genes with 5pUTR OR 3pUTR", gff.get_num_genes_with_5pUTR_OR_3pUTR()
    ...
    if args.prefix:
        ...
        if args.maker:
            ...
            with open(args.prefix+'-AED-stats-per-gene.txt', 'w') as f:
                write(f, gff.get_AED_stats_per_gene())
    return

def output(args, gff):
    if args.AED_per_gene and args.maker:
        print '\n'.join(gff.get_AED_stats_per_gene())
    else:
        default_output(args, gff)
    return
