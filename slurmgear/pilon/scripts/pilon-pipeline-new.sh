...
if $CLEAN && $MARK; then
  cd mreads
  CRDONE=`sbatch -J ${BASE}_cleanreads --dependency=${CLEAN1DEP} -o ${OUT}/cleanreads.slurm.%A.out --mem=2g --time=2:00:00 -c 2 --account=${QOS} ${SCRIPTS}/cleanreads.sh | awk '{print $4}'`
  ##cleanreads.sh
  cd ../
fi
