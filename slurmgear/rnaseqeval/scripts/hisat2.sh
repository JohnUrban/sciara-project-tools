...
samtools view -bSh -q 0 -F 256 ${PRE}.bam | bedtools bamtobed -bedpe -i - 2> bamtobed.err | awk 'OFS="\n" {diff+=$1!=$4; same+=$1==$4; diff2+=$1!=$4&&$8>=2; atleastonemapped+=$1!="."||$4!="."; bothmapped+=$1!="."&&$4!="."; bothmappeddiff+=$1!="."&&$4!="."&&$1!=$4; total+=1; MAPQ+=$8}END{print "same_contig\t"same, "different_contig\t"diff, "different_contig_both_mapped\t"bothmappeddiff, "different_contig_qge2\t"diff2, "only_one_mapped\t"atleastonemapped-bothmapped, "both_mapped\t"bothmapped, "total_pairs\t"total, "pct_pairs_same_ctg\t"100.0*same/total, "pct_pairs_not_same_ctg\t"100.0*diff/total, "pct_pairs_with_1-2mappedmates_not_same_ctg\t"100.0*diff/(atleastonemapped), "pct_pairs_bothmapped_not_same_ctg\t"100.0*bothmappeddiff/bothmapped, "pct_pairs_not_same_ctg_qge2\t"100.0*diff2/total, "pct_bothmapped_not_same_ctg_qge2\t"100.0*diff2/bothmapped, "pct_pairs_with_2mappedmates_not_same_ctg_qge2\t"100.0*diff/(bothmapped), "sum_min_mapq\t"MAPQ, "avg_min_mapq_all_pairs\t"MAPQ/total, "avg_min_mapq_pairs_bothmatesmapped\t"MAPQ/bothmapped, "overall_alignment_rate\t"100.0*(bothmapped*2+(atleastonemapped-bothmapped))/(total*2)}' > info.txt
