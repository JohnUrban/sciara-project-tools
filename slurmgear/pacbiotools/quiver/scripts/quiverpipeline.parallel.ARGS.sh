...
if $QUIVER; then
 if $SORT; then
  CLEANDEP=`sbatch -J ${ASMOUTPREFIX}_quiver --dependency=afterok:${QUIVDEP} -o ${SLURMOUTDIR}/quiver.slurm.%A.out --mem=$QMEM --time=$QTIME -c $QTHREADS --qos=$QQOS --export=THREADS=${QTHREADS},INPUT=${MERGEDCMP},REF=${REF},OUTGFF=${OUTGFF},OUTFASTQ=${OUTFASTQ},OUTFASTA=${OUTFASTA} ${PATH_TO_SCRIPTS}/quiver.sh | awk '{print $4}'`
 else
  CLEANDEP=`sbatch -J ${ASMOUTPREFIX}_quiver -o ${SLURMOUTDIR}/quiver.slurm.%A.out --mem=$QMEM --time=$QTIME -c $QTHREADS --qos=$QQOS --export=THREADS=${QTHREADS},INPUT=${MERGEDCMP},REF=${REF},OUTGFF=${OUTGFF},OUTFASTQ=${OUTFASTQ},OUTFASTA=${OUTFASTA} ${PATH_TO_SCRIPTS}/quiver.sh | awk '{print $4}'`
 fi
fi
